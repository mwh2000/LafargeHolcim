...
require_once __DIR__ . '/../../controllers/energyTypeController.php';
...
$energyTypeController = new EnergyTypeController($conn);
...
try {
    switch ($method) {
        ...
        case 'POST':
            if ($action === 'create') {
                $res = $energyTypeController->create($input);
                sendJson($res);
            } else {
        ...
        case 'GET':
            if ($action === 'all') {
                ...
                $res = $energyTypeController->getAll($filters);
                sendJson($res);
            } elseif ($action === 'show' && isset($queryParams['id'])) {
                $res = $energyTypeController->getById((int) $queryParams['id']);
                sendJson($res);
            } else {
                http_response_code(400);
        ...
        case 'PUT':
            if ($action === 'update' && isset($queryParams['id'])) {
                $res = $energyTypeController->update((int) $queryParams['id'], $input);
                sendJson($res);
            } else {
                http_response_code(400);
        ...
        case 'DELETE':
            if ($action === 'delete' && isset($queryParams['id'])) {
                $res = $energyTypeController->delete((int) $queryParams['id']);
                sendJson($res);
            } else {
                http_response_code(400);
